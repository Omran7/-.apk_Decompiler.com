...
public enum zzaks {
    ...
    private static final zzaks[] zzaz = null;
    private final int zzbb;

    static {
        zzaks[] values = values();
        zzaz = new zzaks[values.length];
        for (zzaks zzaks : values) {
            zzaz[zzaks.zzbb] = zzaks;
        }
    }

    private zzaks(int i2, zzaku zzaku, zzalh zzalh) {
        this.zzbb = i2;
        int ordinal = zzaku.ordinal();
        if (ordinal == 1) {
            zzalh.zza();
        } else if (ordinal == 3) {
            zzalh.zza();
        }
        if (zzaku == zzaku.SCALAR) {
            int i5 = zzakv.zza[zzalh.ordinal()];
        }
    }
